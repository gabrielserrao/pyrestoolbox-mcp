...
\begin{abstract}
This document provides a comprehensive reference guide for all calculation tools available in the pyResToolbox Model Context Protocol (MCP) server. It includes mathematical formulas, symbol definitions, unit systems, and correlation methods for 47 production-ready tools covering oil PVT, gas PVT, well performance, simulation support, brine properties, and reservoir heterogeneity analysis. All calculations use Field Units (US Oilfield standard).
...
\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=\textbf{DISCLAIMER}]
...
Based on a tuned 5-component Peng-Robinson EOS model. Formulation details are proprietary, but the correlation is implemented in pyResToolbox for practical use.
...
\subsection{Brine Density ($\rho_w$)}

\subsubsection{Description}
Brine density depends on pressure, temperature, and salinity (NaCl concentration).

\begin{tcolorbox}[colback=formulabox,colframe=black!75]
\textbf{Pure Water Density:}
\begin{equation}
\rho_{w,pure} = 62.368 + 0.438603 \times 10^{-3} P + f(T)
\end{equation}

\textbf{Brine Density with Salinity:}
\begin{equation}
\rho_w = \rho_{w,pure} + \Delta\rho_{salt}(w_t)
\end{equation}

where salinity correction is:
\begin{equation}
\Delta\rho_{salt} = w_t \times (a_0 + a_1 T + a_2 P)
\end{equation}
\end{tcolorbox}

\textbf{Symbol Definitions:}
\begin{itemize}[leftmargin=2cm]
\symitem{$\rho_w$}{Brine density (lb/ft³)}
\symitem{$\rho_{w,pure}$}{Pure water density (lb/ft³)}
\symitem{$P$}{Pressure (psia)}
\symitem{$T$}{Temperature (°F)}
\symitem{$w_t$}{Salinity (wt\% NaCl)}
\symitem{$a_0, a_1, a_2$}{Correlation coefficients}
\end{itemize}

\subsection{Brine Viscosity ($\mu_w$)}

\subsubsection{Description}
Brine viscosity decreases with temperature and increases with pressure and salinity.

\begin{tcolorbox}[colback=formulabox,colframe=black!75]
\begin{equation}
\mu_w = \mu_{w,pure} \times [1 + A \times w_t + B \times w_t^2]
\end{equation}

where pure water viscosity is:
\begin{equation}
\mu_{w,pure} = a \times 10^{b/(T+c)}
\end{equation}
\end{tcolorbox}
...
\symitem{$\mu_{w,pure}$}{Pure water viscosity (cP)}
\symitem{$w_t$}{Salinity (wt\% NaCl)}
\symitem{$T$}{Temperature (°F)}
\symitem{$A, B, a, b, c$}{Correlation coefficients}
\end{itemize}

\subsection{Brine Compressibility ($c_w$)}

\subsubsection{Description}
Brine compressibility is the fractional change in volume with pressure.

\begin{tcolorbox}[colback=formulabox,colframe=black!75]
\begin{equation}
c_w = \frac{1}{B_w} \frac{dB_w}{dP} \quad \text{(1/psi)}
\end{equation}

\textbf{Empirical Correlation:}
\begin{equation}
c_w = (c_1 + c_2 T + c_3 T^2)(1 + c_4 w_t)
\end{equation}
\end{tcolorbox}
...
\symitem{$T$}{Temperature (°F)}
\symitem{$w_t$}{Salinity (wt\% NaCl)}
\symitem{$c_1, c_2, c_3, c_4$}{Correlation coefficients}
\end{itemize}

\subsection{Brine Formation Volume Factor ($B_w$)}

\begin{tcolorbox}[colback=formulabox,colframe=black!75]
\begin{equation}
B_w = (1 + \Delta V_T)(1 + \Delta V_P)(1 + \Delta V_s)
\end{equation}
where:
\begin{align}
\Delta V_T &= -1.0001 \times 10^{-2} + 1.33391 \times 10^{-4} T + 5.50654 \times 10^{-7} T^2 \\
\Delta V_P &= -1.95301 \times 10^{-9} P T - 1.72834 \times 10^{-13} P^2 T - 3.58922 \times 10^{-7} P \\
\Delta V_s &= -w_t (2.65 \times 10^{-4} - 1.878 \times 10^{-6} w_t)
\end{align}
\end{tcolorbox}
...
\begin{itemize}[leftmargin=2cm]
\symitem{$B_w$}{Water formation volume factor (rb/stb)}
\symitem{$\Delta V_T$}{Thermal expansion effect}
\symitem{$\Delta V_P$}{Pressure effect}
\symitem{$\Delta V_s$}{Salinity effect}
\symitem{$P$}{Pressure (psia)}
\symitem{$T$}{Temperature (°F)}
\symitem{$w_t$}{Salinity (wt\% NaCl)}
\end{itemize}

\subsection{Gas Solubility in Brine ($R_w$)}

\subsubsection{Description}
Volume of gas (CH$_4$ or CO$_2$) dissolved in brine at reservoir conditions.

\begin{tcolorbox}[colback=formulabox,colframe=black!75]
\textbf{Methane Solubility:}
\begin{equation}
R_w = A(T,P) \times (1 - \alpha \times w_t) \quad \text{(scf/stb)}
\end{equation}

where $A(T,P)$ is calculated from Henry's Law with temperature and pressure dependence.
\end{tcolorbox}

\textbf{Symbol Definitions:}
\begin{itemize}[leftmargin=2cm]
\symitem{$R_w$}{Gas solubility in brine (scf/stb)}
\symitem{$A(T,P)$}{Solubility function of temperature and pressure}
\symitem{$\alpha$}{Salinity correction factor}
\symitem{$w_t$}{Salinity (wt\% NaCl)}
\end{itemize}
...
\begin{itemize}
\item \textbf{Production Ready}: All 47 tools tested and validated
...
\item Valko, P.P. and McCain, W.D. (2003). "Reservoir Oil Bubblepoint Pressures Revisited; Solution Gas-Oil Ratios and Surface Gas Specific Gravities." \textit{Journal of Petroleum Science and Engineering}.
\item Velarde, J., Blasingame, T.A., and McCain, W.D. (1997). "Correlation of Black Oil Properties at Pressures Below Bubble Point Pressure." \textit{SPE Annual Technical Conference}.
...
\item Wang, X., Ye, F., and Wu, K. (2021). "A New Correlation for Calculating Gas Compressibility Factor." \textit{Journal of Petroleum Science and Engineering}.
...
\item Piper, L.D., McCain, W.D., and Corredor, J.H. (1993). "Compressibility Factors for Naturally Occurring Petroleum Gases." \textit{SPE Gas Technology Symposium}.
...
\begin{itemize}
\item Corey, A.T. (1954). "The Interrelation Between Gas and Oil Relative Permeabilities." \textit{Producers Monthly}.
...
\begin{itemize}
\item McCain, W.D. (1990). \textit{The Properties of Petroleum Fluids}. PennWell Publishing Company.
\item Batzle, M. and Wang, Z. (1992). "Seismic Properties of Pore Fluids." \textit{Geophysics}.
\end{itemize}
